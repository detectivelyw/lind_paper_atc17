...
In our \lip design, one key component is the sandbox kernel (\ding{172} in Figure \ref{fig:design_safe_reimplementation}), 
...
The principle of the design is to include only simple and necessary system calls with basic flags, and then we can check and verify 
if only ``popular paths'' were used.  
The sandbox kernel should start with building-block functions to first form a minimum set of system calls. 
For example, for network programs, opening a TCP connection is an essential function. 
...
Repy is one possible implementation of the sandbox kernel in our \lip design. 
It starts with basic building-block functions and tries to be conservative. 
Repy was designed and implemented before our ``popular paths'' study. 
It was not a perfect match to the ``popular paths'', but was verified to use only part of the ``popular paths''. 
